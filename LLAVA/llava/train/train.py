...
class TrainingArguments(transformers.TrainingArguments):
    ...
    unfreeze_vision_tower_layers: bool = field(default=False)
...
def preprocess_v1(
    ...
    for conversation, target in zip(conversations, targets):
        ...
        for i, rou in enumerate(rounds):
            ...
            if i == len(rounds) - 2:  # last round, keep answer for training
                target[cur_len: cur_len + instruction_len] = IGNORE_INDEX
            else:
                target[cur_len: cur_len + round_len] = IGNORE_INDEX  # previous rounds - mask everything
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        if self.data_args.do_augment:
            self.augment = TrivialAugmentWide(strength=0.5)  # 0.2 weak, 0.5 strong
        else:
            self.augment = None
    ...
    def load_image_biovil(self, image_folder, image_file) -> Image.Image:
        ...
        if image_file.startswith('/home'):  # full path
            path = pathlib.Path(image_file)
        elif image_file.startswith('files/'):  # mimic-cxr
            path = pathlib.Path(os.path.join(image_folder, image_file))
        else:
            path = pathlib.Path("/home/guests/chantal_pellegrini/" + image_file)  # radrestruct
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0]:  # 1 or multiple current images
            image_files = self.list_data_dict[i]['image'] if type(self.list_data_dict[i]['image']) == list else [
                self.list_data_dict[i]['image']]  # convert to list
            image_folder = self.data_args.image_folder

            if self.do_img_order_augment:
                random.shuffle(image_files)
                n_images = random.randint(1, len(image_files))
                image_files = image_files[:n_images]

            if self.data_args.vision_tower == 'biovil':
                images = [self.load_image_biovil(image_folder, image_file) for image_file in image_files]
                # augment images
                if self.augment is not None:
                    images = [self.augment(image) for image in images]
                images = [self.vis_transforms_biovil(img) for img in images]
            else:
                ...
                if self.data_args.image_aspect_ratio == 'pad':
                    ...
                    images = [processor.preprocess(image, return_tensors='pt')['pixel_values'][0] for image in images]
            # stack images
            images = torch.stack(images, dim=0).squeeze()  # stack and drop unnecessary dimension

            if 'prev_image' in sources[0]:  # 1 or multiple previous images
                raise NotImplementedError("Previous image is not supported yet")
                prev_image_files = self.list_data_dict[i]['prev_image'] if type(self.list_data_dict[i]['prev_image']) == list else [
                    self.list_data_dict[i]['prev_image']]  # convert to list
                image_folder = self.data_args.image_folder

                if self.data_args.vision_tower == 'biovil':
                    prev_images = [self.load_image_biovil(image_folder, image_file) for image_file in prev_image_files]

                    prev_images = [self.vis_transforms_biovil(img) for img in prev_images]
                else:
                    processor = self.data_args.image_processor
                    prev_images = [Image.open(os.path.join(image_folder, image_file)).convert('RGB') for image_file in prev_image_files]
                    if self.data_args.image_aspect_ratio == 'pad':
                        def expand2square(pil_img, background_color):
                            width, height = pil_img.size
                            if width == height:
                        ...
                        prev_images = [expand2square(image, tuple(int(x * 255) for x in processor.image_mean)) for image in prev_images]
                        prev_images = [processor.preprocess(image, return_tensors='pt')['pixel_values'][0] for image in prev_images]
                    else:
                        prev_images = [processor.preprocess(image, return_tensors='pt')['pixel_values'][0] for image in prev_images]

                # stack images
                prev_images = torch.stack(prev_images, dim=0).squeeze() if len(
                    prev_images) > 0 else prev_images  # stack and drop unnecessary dimension

            # drop images from the data
        ...
        if 'image' in self.list_data_dict[i]:
            data_dict['image'] = images[0] if len(images) == 1 else images
            if 'prev_image' in self.list_data_dict[i]:
                data_dict['prev_image'] = prev_images[0] if len(prev_images) == 1 else prev_images
                if prev_images == []:
                    data_dict['prev_image'] = None
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in instances[0]:
            ...
            if not 'prev_image' in instances[0] and all(x is not None and x.shape == images[0].shape for x in images):
                batch['images'] = torch.stack(images)
            else:
                # extend the dimension of all images to 4 if it is only 3 (1x dimension to be treated as multi-image)
                images = [image.unsqueeze(0) if len(image.shape) == 3 else image for image in images]
                batch['images'] = images

        if 'prev_image' in instances[0]:
            prev_images = [instance['prev_image'] for instance in instances]
            prev_images = [image.unsqueeze(0) if len(image.shape) == 3 else image for image in prev_images]
            batch['prev_images'] = prev_images
...
def train():
    ...
    data_args.vision_tower = model_args.vision_tower  # info for image preprocessing
    ...
    if model_args.vision_tower is not None:
        ...
        if model.get_model().image_pooler is not None:
        ...
        if training_args.unfreeze_vision_tower_layers:
            print(f'Unfreezing all vision tower layers')
